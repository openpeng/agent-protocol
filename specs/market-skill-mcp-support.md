...
### 2.1 agent.json 中的 Skills 格式

市场支持以下 skills 定义来源：

**格式 A — agent.json 顶层 `skills` 数组**（agent-builder 平铺格式）：
...
**格式 C — skills/ 目录下的 skill.json 文件（v3.1 规范推荐）**
...
**来源**：Agent 包内 `skills/{name}/skill.json` 文件。每个 `skill.json` 是独立的 Skill 元数据文件，包含 `identity`、`content`、`capabilities` 等字段。市场提取时扫描 `skills/` 目录下的所有 `skill.json` 文件。

> **注**：三种格式共存时按 Skill name 去重合并，市场提取时不做取舍。当前 agent-builder 主要使用格式 A，格式 B 为 Runtime 文件系统加载方式，格式 C 为 v3.1 规范推荐的独立 Skill 元数据方式。
...
2. 如果包内有 `skills/*.yaml` 文件 → 扫描文件名，加上 `{agent_name}/` 前缀
3. 如果包内有 `skills/*/skill.json` 文件 → 解析 skill.json，name 加上 `{agent_name}/` 前缀
...
class SkillInfo(BaseModel):
    ...
    category: str = ""
    icon: str = ""
    content: str = ""        # Skill 指令内容（从 skill.json 或 SKILL.md 提取）
...
# ---- skills/ 目录下的 skill.json 校验 ----
if pkg_path.exists():
    skills_dir = pkg_path / "skills"
    if skills_dir.is_dir():
        for skill_json_file in skills_dir.rglob("skill.json"):
            try:
                skill_cfg = json.loads(skill_json_file.read_text(encoding="utf-8"))
                identity = skill_cfg.get("identity", {})
                if not identity.get("name"):
                    errors.append(f"skill.json 缺少 identity.name: {skill_json_file}")
                if not identity.get("version"):
                    errors.append(f"skill.json 缺少 identity.version: {skill_json_file}")
            except json.JSONDecodeError:
                errors.append(f"skill.json 不是有效的 JSON: {skill_json_file}")
```
...
def _extract_skills_info(metadata: dict, extract_dir: Path = None) -> list[dict]:
    ...
    # 格式 A: agent.json 顶层 skills 数组（agent-builder 使用）
        ...
        if skills_dir.is_dir():
            for skill_json_file in skills_dir.rglob("skill.json"):
                try:
                    ...
                    identity = skill_meta.get("identity", {})
                    raw_name = identity.get("name", "")
                    if raw_name and raw_name not in seen_names:
                        content_cfg = skill_meta.get("content", {})
                        content = ""
                        if content_cfg.get("source") == "file" and content_cfg.get("file"):
                            content_file = skill_json_file.parent / content_cfg["file"]
...
- [ ] 格式 A（agent-builder skills 数组）提取正确，id 含 agent 前缀
- [ ] 格式 B（skills/*.yaml 文件系统）提取正确
- [ ] 格式 C（skills/*/skill.json 文件系统）提取正确，content 和 capabilities 字段完整
...
## 9. Skill/MCP 独立打包与引用（v1.1 扩展）

> 基于 [Skill & MCP 独立打包与引用机制 SPEC](../../SPEC_skill_mcp_reference.md) 扩展，支持 Skill 和 MCP Server 作为独立包上传、下载和被 Agent 引用。

### 9.1 Skill 独立包上传

新增 `POST /api/v1/skills/upload` 端点，支持通过 multipart/form-data 上传 Skill 包（tar.gz 格式）。
...
curl -X POST /api/v1/skills/upload \
  -H "Authorization: Bearer $API_KEY" \
  -F "file=@html-anything-skill-v1.0.0.tar.gz"
```

**处理逻辑**：

1. 接收 tar.gz 文件，解压到临时目录
2. 验证 `skill.json` 存在且格式合法（schema_version、identity 必填字段）
3. 验证 `SKILL.md` 存在且非空（当 skill.json 中 content.source 为 "file" 时）
4. 提取 `skill.json` 中的 identity、content、capabilities 信息，写入 `skills` 表
5. 存储 tar.gz 到 `packages/skills/{id}/` 目录，记录 `package_path` 和 `package_size`
6. 缓存 SKILL 内容到 `content` 字段（从 SKILL.md 文件读取或取 content.content 内联内容）
7. 返回 Skill 详情（含 id、version、download_url）
...
```json
{
  ...
  "id": "html-anything",
  "version": "1.0.0",
  "download_url": "/api/v1/skills/html-anything/download"
}
```

### 9.2 Skill 包下载

新增 `GET /api/v1/skills/{id}/download` 端点，返回 Skill 的 tar.gz 包。

```bash
curl -o html-anything.tar.gz \
  https://market.aitboy.cn/api/v1/skills/html-anything/download
```

**处理逻辑**：

1. 查询 `skills` 表获取 `package_path`
2. 如果 `package_path` 为空（仅元数据注册），返回 404 并提示无可用包
3. 返回 tar.gz 文件（Content-Type: application/gzip）

**查询参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| `version` | string | 可选，指定版本（默认最新） |

### 9.3 MCP Server 独立包上传

新增 `POST /api/v1/mcp-servers/upload` 端点，支持通过 multipart/form-data 上传 MCP Server 包（tar.gz 格式）。

**请求格式**：

```bash
curl -X POST /api/v1/mcp-servers/upload \
  -H "Authorization: Bearer $API_KEY" \
  -F "file=@tapd-mcp-server-v1.0.0.tar.gz"
```

**处理逻辑**：

1. 接收 tar.gz 文件，解压到临时目录
2. 验证 `mcp-server.json` 存在且格式合法
3. 验证 `mcp-config.json` 存在（Claude Desktop 兼容格式）
4. 提取 identity 信息，写入 `mcp_servers` 表
5. 存储 tar.gz 到 `packages/mcp-servers/{id}/` 目录
6. 缓存 `mcp-config.json` 内容到 `config_content` 字段
7. 返回 MCP Server 详情

### 9.4 MCP Server 包下载

新增 `GET /api/v1/mcp-servers/{id}/download` 端点，返回 MCP Server 的 tar.gz 包。

```bash
curl -o tapd-mcp-server.tar.gz \
  https://market.aitboy.cn/api/v1/mcp-servers/tapd/download
```

**处理逻辑**：与 Skill 包下载类似，查询 `mcp_servers` 表获取 `package_path` 后返回文件。

### 9.5 数据库扩展

在 v1.0 的 4 张表基础上，扩展 `skills` 和 `mcp_servers` 表以支持包存储：

```sql
-- skills 表扩展字段
ALTER TABLE skills ADD COLUMN package_path TEXT;          -- tar.gz 存储路径
ALTER TABLE skills ADD COLUMN package_size INTEGER DEFAULT 0;  -- 包大小（字节）
...
-- mcp_servers 表扩展字段
ALTER TABLE mcp_servers ADD COLUMN package_path TEXT;      -- tar.gz 存储路径
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | TEXT PK | qualified name: "agent-name/skill-name" 或独立注册名 |
| `original_name` | TEXT | 原始 name |
| `display_name` | TEXT | 展示名称 |
| `description` | TEXT | 描述 |
| `version` | TEXT | 版本号 |
| `category` | TEXT | 分类 |
| `icon` | TEXT | 图标 |
| `package_path` | TEXT | tar.gz 包存储路径（v1.1 新增） |
| `package_size` | INTEGER | 包大小（v1.1 新增） |
| `package_format` | TEXT | 包格式（v1.1 新增） |
| `content_format` | TEXT | 内容格式（v1.1 新增） |
| `content_source` | TEXT | 内容来源（v1.1 新增） |
| `content` | TEXT | SKILL.md 内容缓存（v1.1 新增） |
| `created_at` | TEXT | 创建时间 |
| `updated_at` | TEXT | 更新时间 |

**扩展后 mcp_servers 表完整结构**：

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | TEXT PK | qualified name |
| `original_name` | TEXT | 原始 name |
| `description` | TEXT | 描述 |
| `command` | TEXT | 启动命令 |
| `args` | TEXT | 启动参数（JSON array） |
| `package` | TEXT | NPM 包名 |
| `tools` | TEXT | 工具列表（JSON array） |
| `required_env` | TEXT | 必需环境变量（JSON array） |
| `package_path` | TEXT | tar.gz 包存储路径（v1.1 新增） |
| `package_size` | INTEGER | 包大小（v1.1 新增） |
| `package_format` | TEXT | 包格式（v1.1 新增） |
| `config_content` | TEXT | mcp-config.json 内容缓存（v1.1 新增） |
| `created_at` | TEXT | 创建时间 |
| `updated_at` | TEXT | 更新时间 |

### 9.6 打包函数

**`pack_skill()`** — 将 Skill 目录打包为 tar.gz：

```python
def pack_skill(skill_dir: Path, output: Path = None) -> Path:
    """
    将 Skill 目录打包为 tar.gz

    Args:
        skill_dir: Skill 目录路径（包含 skill.json、SKILL.md）
        output: 输出文件路径（默认: {name}-v{version}.tar.gz）

    Returns:
        生成的 tar.gz 文件路径

    Raises:
        ValueError: 缺少 skill.json 或 SKILL.md
    """
    skill_json = skill_dir / "skill.json"
    skill_md = skill_dir / "SKILL.md"

    if not skill_json.exists():
        raise ValueError(f"缺少 skill.json: {skill_json}")
    if not skill_md.exists():
        raise ValueError(f"缺少 SKILL.md: {skill_md}")
    ...
    # 打包
    output = output or Path(f"{name}-v{version}.tar.gz")
    prefix = f"{name}-v{version}/"

    with tarfile.open(output, "w:gz") as tar:
        for item in skill_dir.rglob("*"):
            if item.is_file() and item.name != output.name:
                arcname = prefix + str(item.relative_to(skill_dir))
                tar.add(item, arcname=arcname)

    return output
```

**`pack_mcp_server()`** — 将 MCP Server 目录打包为 tar.gz：

```python
def pack_mcp_server(mcp_dir: Path, output: Path = None) -> Path:
    """
    将 MCP Server 目录打包为 tar.gz

    Args:
        mcp_dir: MCP Server 目录路径（包含 mcp-server.json、mcp-config.json）
        output: 输出文件路径（默认: {name}-v{version}.tar.gz）
    ...
    Raises:
        ValueError: 缺少 mcp-server.json 或 mcp-config.json
    """
    server_json = mcp_dir / "mcp-server.json"
    config_json = mcp_dir / "mcp-config.json"

    if not server_json.exists():
        raise ValueError(f"缺少 mcp-server.json: {server_json}")
    if not config_json.exists():
        raise ValueError(f"缺少 mcp-config.json: {config_json}")
    ...
    with tarfile.open(output, "w:gz") as tar:
        for item in mcp_dir.rglob("*"):
            if item.is_file() and item.name != output.name:
                arcname = prefix + str(item.relative_to(mcp_dir))
                tar.add(item, arcname=arcname)
...
├── SKILL.md            # Skill 指令内容（当 content.source=file 时必需）
├── scripts/            # 可执行脚本（可选）
...
- 必须包含 `skill.json`
- 当 `content.source` 为 `"file"` 时必须包含 `SKILL.md`
- 当 `content.source` 为 `"inline"` 时内容直接存储在 `content.content` 字段中
- 可选包含 `scripts/`、`templates/`、`README.md`
- 总大小不超过 10MB

### 9.8 MCP Server 包结构

MCP Server 独立包的标准目录结构：
...
**mcp-config.json 格式**（Claude Desktop 兼容）：
...
### 9.9 API 端点汇总（v1.1 新增）

| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| `POST` | `/api/v1/skills/upload` | publisher | 上传 Skill 包（multipart/form-data, tar.gz） |
| `GET` | `/api/v1/skills/{id}/download` | — | 下载 Skill 包（返回 tar.gz） |
| `POST` | `/api/v1/mcp-servers/upload` | publisher | 上传 MCP Server 包（multipart/form-data, tar.gz） |
| `GET` | `/api/v1/mcp-servers/{id}/download` | — | 下载 MCP Server 包（返回 tar.gz） |

> **注**：v1.0 已有的 `POST /api/v1/skills`（JSON Body 独立注册）和 `POST /api/v1/mcp-servers`（JSON Body 独立注册）端点保持不变，两种注册方式共存。
