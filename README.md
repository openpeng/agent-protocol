...
6. **Skill & MCP 市场引用** - 独立打包、市场引用、运行时解析与缓存（v3.1 新增）
7. **Deployment Targets** - 跨 9 平台部署适配
...
### ✅ 4. Skill & MCP 市场引用（v3.1 新增）

**Skill 独立打包与引用**：
- Skill 使用 `skill.json` 独立定义，支持 `skill pack/upload/download`
- Agent 通过 `skills` 数组引用市场 Skill：`ref` + `version` + `market_url`
- 运行时自动解析版本约束（`^`、`~`、`>=`、`*`）、下载并缓存

**MCP Server 独立打包与引用**：
- MCP Server 使用 `mcp-server.json` 独立定义
- Agent 通过 `mcp_servers` 数组引用市场 MCP Server
- 支持内联模式、引用模式、混合模式

**缓存管理**：
- 本地缓存目录：`~/.agent-hub/cache/skills/`、`~/.agent-hub/cache/mcp-servers/`
- CLI 命令：`agent-deploy cache status`、`agent-deploy cache clean`、`agent-deploy cache update`
...
| [specs/market-skill-mcp-support.md](./specs/market-skill-mcp-support.md) | 市场 Skill & MCP 支持规范（v3.1） |
